...
<head>
  ...
  <title>ProductosSerigraficos</title>
    ...
    <style>
    /* Estilos para las opciones del chatbot */
        .chat-options {
            display: flex;
        ...
        .btn-option {
            padding: 8px 12px;
            background: linear-gradient(135deg, #189561 0%, #03A9F4 100%);
            color: white;
            border: none;
            border-radius: 20px;
            cursor: pointer;
        ...
        /* Estilos para mensajes con múltiples líneas */
        .chatbot-message {
        ...
        .chatbot-window {
           width: 350px;
            ...
            display: none;
            flex-direction: column;
            overflow: hidden; /* Esto se mantiene para el contenedor principal */
            position: absolute;
            bottom: 80px;
            right: 0;
        }
        ...
        .chatbot-body {
            ...
            overscroll-behavior: contain; /* Mejor comportamiento en móviles */
            -webkit-overflow-scrolling: touch; /* Scroll suave en iOS */
            max-height: calc(100% - 120px); /* Ajusta según el header y footer */
        }
        ...
        @keyframes pulse {
            0% {
                box-shadow: 0 0 0 0 rgba(24, 149, 97, 0.7);
            }
            70% {
                box-shadow: 0 0 0 10px rgba(24, 149, 97, 0);
            }
            100% {
                box-shadow: 0 0 0 0 rgba(24, 149, 97, 0);
            }
        }
...
<div class="table-responsive">
    <table class="table table-hover" id="tablaProductosSerigraficos">
        ...
        <tbody>
            <?php 
            // Verificar si la sesión está iniciada y obtener el ID del cliente
            $idCliente = isset($_SESSION['session_id']) ? $_SESSION['session_id'] : 0;
            $pedidos = $this->model->ObtenerPedidosCliente($idCliente);
            
            foreach($pedidos as $r): 
                // Convertir estado numérico a texto para visualización
            $estadoTexto = '';
            $badgeClass = '';
            ...
            switch ($r->estado) {
                ...
                case 1:
                    ...
                    break;
                case 2:
                    $estadoTexto = 'Falta Pagar Con QR';
                    $badgeClass = 'badge-warning'; // Puedes cambiar el color según tus necesidades
                    break;
                case 3:
                    $estadoTexto = 'Pedido Confirmado';
                    $badgeClass = 'badge-info'; // Puedes cambiar el color según tus necesidades
                    break;
                case 4: // Para el caso de Cancelado o cualquier otro valor no especificado
                    $estadoTexto = 'Falta Pagar Saldo QR';
                    $badgeClass = 'badge-danger';
                    break;
                case 5: // Para el caso de Cancelado o cualquier otro valor no especificado
            ...
            <tr>
                ...
                <td>
                    ...
                    <a href="#" class="img-popup" data-img="<?php echo $r->imagen; ?>">
                        <img src="data:image/jpeg;base64,<?php echo $r->imagen; ?>" 
                ...
                <td class="text-center">
                    ...
                    <button class="btn btn-sm btn-warning btn-confirmar-adelanto" 
                    ...
                    <?php if($r->estado == 1): ?>
                        <!-- Botón para atender pedido -->
                    <button class="btn btn-sm btn-info btn-atender" 
                            data-id="<?php echo $r->id; ?>" 
                            title="Desea cancelar el Pedido">
                        <i class="fas fa-check-circle"></i> Pendiente de Cotización
                    </button>
                    <?php endif; ?>


                    <?php if($r->estado == 0): ?>
                        <!-- Botón para habilitar pedido (estado 0) -->
...
<!-- Modal Subir Comprobante de Adelanto -->
<div class="modal fade" id="modalSubirComprobante" tabindex="-1" role="dialog" aria-hidden="true">
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            ...
            <form id="formSubirComprobante" method="post" enctype="multipart/form-data">
                <div class="modal-body">
                    <input type="hidden" name="id_pedido" id="id_pedido_adelanto">
                    
                    <!-- Sección Comprobante -->
                    <div class="form-group">
                        <label for="comprobante_adelanto">Subir comprobante de pago :</label>
                        <div class="custom-file">
                            <input type="file" class="custom-file-input" id="comprobante_adelanto" name="comprobante_adelanto" accept="image/*,.pdf" required>
                            <label class="custom-file-label" for="comprobante_adelanto">Seleccionar comprobante...</label>
                        </div>
                        <small class="form-text text-muted">Formatos aceptados: JPG, PNG, PDF (Máx. 2MB)</small>
                        <div class="mt-2" id="preview_comprobante"></div>
                    </div>
    ...
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="modalCambioEstadoTitle">Confirmar Cambio de Estado</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <p id="mensajeConfirmacion">¿Está seguro que desea cambiar el estado de este pedido?</p>
                <input type="hidden" id="pedidoIdCambio">
...
<!-- Modal para imagen ampliada - Debe estar al final del body -->
    ...
    <div class="modal-dialog modal-dialog-centered-top modal-lg">
        <div class="modal-content">
            ...
            <div class="modal-body text-center p-0">
                <img id="imagenAmpliada" src="" class="img-fluid" style="max-height: 80vh; width: auto;">
            </div>
            <div class="modal-footer">
...
$(document).ready(function() {
        ...
        // Manejar clic en el botón de Confirmar Adelanto
    $('.btn-confirmar-adelanto').click(function() {
        var idPedido = $(this).data('id');
        $('#id_pedido_adelanto').val(idPedido);
    ...
    $('#comprobante_adelanto').change(function(e) {
        if (this.files && this.files[0] && this.files[0].type.match('image.*')) {
            var reader = new FileReader();
            reader.onload = function(e) {
                $('#preview_comprobante').html('<img src="'+e.target.result+'" class="img-thumbnail" style="max-height: 200px;">');
            }
    ...
    $(document).on('click', '.btn-cancelar', function() {
        ...
        Swal.fire({
            ...
            if (result.value) {
                cambiarEstadoPedido(pedidoId, 0);
            }
        });
    });
    
    // Manejar clic en botón habilitar
    $(document).on('click', '.btn-habilitar', function() {
        const pedidoId = $(this).data('id');
        
        Swal.fire({
            title: '¿Habilitar Pedido?',
            text: "¿Está seguro que desea habilitar este pedido?",
            type: 'question',
            showCancelButton: true,
            confirmButtonColor: '#3085d6',
            cancelButtonColor: '#d33',
            ...
            if (result.value) {
                cambiarEstadoPedido(pedidoId, 1);
            }
        });
    });
      $(document).on('click', '.btn-atender', function() {
        ...
        Swal.fire({
            title: '¿Desabilitar el Pedido?',
            text: "¿Está seguro que desea desabilitar este pedido?",
            type: 'question',
            showCancelButton: true,
            confirmButtonColor: '#3085d6',
            cancelButtonColor: '#d33',
    ...
    // Función para limpiar backdrops persistentes
...
$(document).on('click', '.btn-minus', function() {
    ...
    $(document).ready(function() {
        ...
        chatbotBtn.click(function() {
            chatbotWindow.fadeToggle(300);
        });
        
        closeChat.click(function() {
            chatbotWindow.fadeOut(300);
        });
...
function processUserResponse(message) {
    switch(chatState.step) {
        case 0: // Inicio
            startConversation();
            break;
        case 1: // Tipo de pedido
            handleOrderType(message);
            break;
        case 2: // Cantidad
            handleQuantity(message);
            break;
        case 3: // Complejidad
            handleComplexity(message);
            break;
        case 4: // Confirmación
            handleConfirmation(message);
            break;
        case 5: // Método de pago
...
function startConversation() {
    setTimeout(() => {
        chatbotMessages.append(`
            <div class="chatbot-message bot-message">
                ¡Bienvenido al sistema de pedidos de serigrafía!
                <br><br>
                Por favor seleccione el tipo de pedido:
                <div class="chat-options">
                    <button class="btn-option" data-value="camisetas">Camisetas/Textiles</button>
                    <button class="btn-option" data-value="papel">Papel/Stickers</button>
                    <button class="btn-option" data-value="otros">Otros materiales</button>
                </div>
            </div>
        `);
...
// Función para manejar tipo de pedido
function handleOrderType(message) {
    const validOptions = ['camisetas', 'papel', 'otros'];
    if(validOptions.includes(message.toLowerCase())) {
        chatState.orderDetails.type = message.toLowerCase();
        
        setTimeout(() => {
            chatbotMessages.append(`
                <div class="chatbot-message bot-message">
                    Entendido, pedido para ${message}.
                    <br><br>
                    ¿Cuántas unidades necesita?
                    <div class="chat-options">
                        <button class="btn-option" data-value="1-50">1-50 unidades</button>
                        <button class="btn-option" data-value="51-100">51-100 unidades</button>
                        <button class="btn-option" data-value="100+">Más de 100 unidades</button>
                    </div>
                </div>
            `);
...
function handleQuantity(message) {
    const validOptions = ['1-50', '51-100', '100+'];
    if(validOptions.includes(message)) {
        chatState.orderDetails.quantity = message;
        
        setTimeout(() => {
            chatbotMessages.append(`
                <div class="chatbot-message bot-message">
                    Cantidad: ${message} unidades.
                    <br><br>
                    ¿Qué tan complejo es su diseño?
                    <div class="chat-options">
                        <button class="btn-option" data-value="simple">Simple (1-2 colores)</button>
                        <button class="btn-option" data-value="medio">Medio (3-4 colores)</button>
                        <button class="btn-option" data-value="complejo">Complejo (5+ colores)</button>
                    </div>
                </div>
            `);
...
// Función para manejar complejidad
function handleComplexity(message) {
    const validOptions = ['simple', 'medio', 'complejo'];
    if(validOptions.includes(message.toLowerCase())) {
        chatState.orderDetails.complexity = message.toLowerCase();
        
        // Calcular tiempo estimado y precio aproximado
        ...
        setTimeout(() => {
            chatbotMessages.append(`
                <div class="chatbot-message bot-message">
                    Según los datos:
                    <br>- Tipo: ${chatState.orderDetails.type}
                    <br>- Cantidad: ${chatState.orderDetails.quantity}
                    <br>- Complejidad: ${chatState.orderDetails.complexity}
                    <br><br>
                    <strong>Tiempo estimado:</strong> ${estimate.time}
                    <br><strong>Precio aproximado:</strong> ${estimate.price}
                    <br><br>
                    ¿Desea proceder con este pedido?
                    <div class="chat-options">
                        <button class="btn-option" data-value="si">Sí, continuar</button>
                        <button class="btn-option" data-value="no">No, cancelar</button>
                    </div>
                </div>
            `);
    ...
    function calculateEstimate() {
        // Lógica de estimación basada en los datos
        let time = "";
        let price = "";
        
        // Calcular tiempo
        if(chatState.orderDetails.quantity === "1-50") {
            time = "3-5 días hábiles";
        } else if(chatState.orderDetails.quantity === "51-100") {
        ...
        let basePrice = 0;
        if(chatState.orderDetails.type === "camisetas") {
            basePrice = chatState.orderDetails.quantity === "1-50" ? 15 : 
                    chatState.orderDetails.quantity === "Bs.1-100" ? 12 : 10;
        } else if(chatState.orderDetails.type === "papel") {
            basePrice = chatState.orderDetails.quantity === "1-50" ? 8 : 
                    chatState.orderDetails.quantity === "Bs.1-100" ? 6 : 5;
        } else {
            basePrice = chatState.orderDetails.quantity === "1-50" ? 20 : 
                    chatState.orderDetails.quantity === "Bs.1-100" ? 18 : 15;
        }
        
        // Ajustar por complejidad
        if(chatState.orderDetails.complexity === "medio") {
            basePrice *= 1.5;
        } else if(chatState.orderDetails.complexity === "complejo") {
            basePrice *= 2;
        }
        
        // Calcular precio total aproximado
        const minUnits = chatState.orderDetails.quantity === "1-50" ? 50 : 
                    chatState.orderDetails.quantity === "Bs.1-100" ? 100 : 150;
        price = `Bs. ${(basePrice * minUnits).toFixed(2)} (aproximado)`;
    ...
    function handleConfirmation(message) {
        if(message.toLowerCase() === "si") {
            setTimeout(() => {
                chatbotMessages.append(`
                    <div class="chatbot-message bot-message">
                        Perfecto. Para proceder necesitamos un adelanto del 30%.
                        <br><br>
                        ¿Cómo desea realizar el pago?
                        <div class="chat-options">
                            <button class="btn-option" data-value="qr">Pago con QR</button>
                            <button class="btn-option" data-value="transferencia">Transferencia bancaria</button>
                            <button class="btn-option" data-value="efectivo">Efectivo en tienda</button>
                        </div>
                    </div>
                `);
            ...
            endConversation("Pedido cancelado. ¿En qué más puedo ayudarte?");
        }
    }

    // Función para manejar método de pago
    function handlePayment(message) {
        const validOptions = ['qr', 'transferencia', 'efectivo'];
        if(validOptions.includes(message.toLowerCase())) {
            chatState.orderDetails.payment = message.toLowerCase();
            
            setTimeout(() => {
                let paymentInstructions = "";
                
                if(message.toLowerCase() === "qr") {
                    paymentInstructions = `
                        <br><br>
                        <strong>Escanea este código QR para completar tu pago:</strong>
                        <br>
                        <img src="assets/img/qr.jpg" style="width: 150px; margin: 10px 0;">
                        <br>
                        Una vez completado, envíanos el comprobante.
                    `;
                } else if(message.toLowerCase() === "transferencia") {
                    paymentInstructions = `
                        <br><br>
                        <strong>Datos para transferencia:</strong>
                        <br>Banco: Banco Nacional
                        <br>Cuenta: 123-456-789
                        <br>Titular: Serigrafía ISSIS
                        <br>CLABE: 012345678912345678
                        <br>
                        Envíanos el comprobante una vez realizado.
                    `;
                } else {
                    paymentInstructions = `
                        <br><br>
                        Puedes pasar a nuestra tienda en:
                        <br>Av. Perifirica /c. 25 de Julio #14039
                        <br>Horario: 9:00 am - 6:00 pm
                    `;
                }
                ...
                chatbotMessages.append(`
                    <div class="chatbot-message bot-message">
                        ...
                        ${paymentInstructions}
                        <br><br>
                        Nos pondremos en contacto contigo para confirmar los detalles finales.
                        <br><br>
                        <button class="btn-option" id="newOrder">Realizar otro pedido</button>
                    </div>
                `);
                
                // Configurar botón para nuevo pedido
    ...
    function endConversation(message = "Gracias por contactarnos. ¡Hasta luego!") {
        setTimeout(() => {
            chatbotMessages.append(`
                <div class="chatbot-message bot-message">
                    ${message}
                    <br><br>
                    <button class="btn-option" id="newOrder">Iniciar nuevo pedido</button>
                </div>
            `);
    ...
    function showInvalidOption() {
        setTimeout(() => {
            chatbotMessages.append(`
                <div class="chatbot-message bot-message">
                    Opción no válida. Por favor seleccione una de las opciones proporcionadas.
                </div>
            `);
            updateScroll();
        }, 1000);
    ...
    // Función para configurar botones de opciones
    function setupOptionButtons() {
        $('.btn-option').click(function() {
            const value = $(this).data('value');
            chatbotInput.val(value);
            sendMessage();
        });
    }

    // Función para resetear el chatbot
    ...
    // Iniciar el chatbot cuando se abre la ventana
    $(document).ready(function() {
        // ... (código existente para abrir/cerrar el chat)
    ...
    $('#formSubirComprobante').submit(function(e) {
    e.preventDefault();
    ...
    $.ajax({
        url: '?c=pedidos&a=atenderPedidoQrInicial',
        type: 'POST',
        data: formData,
        processData: false,
        contentType: false,
        ...
        success: function(res) {
            // Ahora res ya es un objeto, no necesitamos parsear
            if (res.success) {
                Swal.fire({
                    icon: 'success',
                    title: 'Éxito',
                    text: res.message,
                    confirmButtonColor: '#189561'
                }).then(() => {
                    $('#modalAtenderPedido').modal('hide');
                    location.reload();
                });
            } else {
                Swal.fire({
        ...
        complete: function() {
            $submitBtn.prop('disabled', false)
                     .html('<i class="fas fa-check-circle"></i> Confirmar');
        }
    });
  });
