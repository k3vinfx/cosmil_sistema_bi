            ...
            <div class="modal-body position-relative">
                ...
                <form id="frm-nueva-categoria">
                    <div class="form-group">
                        ...
                        <label for="nueva_nombre">Nombre de la Categoría</label>
                    </div>
                </form>
            </div>
...
$(document).ready(function() {
  ...
  tabla.column(3).search('').draw();
    ...
    // Configuración de validación para formulario NUEVA CATEGORÍA
    $("#frm-nueva-categoria").validate({
        ...
        submitHandler: function(form) {
            ...
            $.ajax({
                ...
                complete: function() {
                    btn.prop('disabled', false).html('<i class="fas fa-save mr-2"></i>Guardar Categoría');
                }
            });
        }
    });
    ...
    $(document).on('change', '.estado-categoria', function() {
        ...
        $.ajax({
            url: '?c=categorias&a=CambiarEstado',
            type: 'POST',
            data: {
                id: id,
                estado: estado
    ...
    $(document).on('click', '.btn-editar', function() {
        ...
        $.ajax({
            url: '?c=categorias&a=ObtenerCategoria',
            type: 'POST',
            data: {id: categoriaId},
            dataType: 'json',
            success: function(response) {
